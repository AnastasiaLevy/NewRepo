+function ($) {
    ...
    WSCT.prototype.showResults = function () {
        ...
        this.resultString.push('#1) ' + this.log[0].strategy + ' - ' + this._getMatches(this.log[0]) + ' - ' + this.log[0].correct + ' - ');
        ...
        for (var i = 1; i < this.log.length; i++) {
            ...
            this.resultString.push('#' + (i + 1) + ') ' + this.log[i].strategy + ' - ' + this._getMatches(this.log[i]) + ' - ' + this.log[i].correct + ' - ' + log_text + ' / ' + principe);
    ...
    WSCT.prototype._SaveCardSortObj = function (params) {
        ...
        jQuery.ajax({
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            url: 'WCSTPage.aspx/SaveHTMLstring',
            dataType: 'json',
    ...
    WSCT.prototype._SendResultRow = function (params) {
        ...
        var data = {
            ...
            'errorCnt': params.error_cnt,
            'errorTime': params.error_cnt != 0 ? Math.trunc(params.error_time / params.error_cnt) : '',
            'persevResp': params.persev_resp,
            'persevTime': params.persev_resp != 0 ? Math.trunc(params.persev_time / params.persev_resp) : '',
            'persevRespError': params.persev_resp_err,
            'persevRespErrTime': params.persev_resp_err != 0 ? Math.trunc(params.persev_resp_err_time / params.persev_resp_err) : '',
            'uniqueErr': params.uniq_err,
